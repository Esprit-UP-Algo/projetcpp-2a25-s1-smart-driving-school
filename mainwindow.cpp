...
void MainWindow::on_pushButton_8_clicked()
{
    ...
    QString filePath = QFileDialog::getSaveFileName(
        this,
        tr("Exporter les véhicules en PDF"),
    ...
    // Column widths
    const int wId = 150;
    const int wMatricule = 250;
    const int wMarque = 250;
    const int wKm = 200;
    const int wEtat = 200;
    const int rowH = 70;

    const int xId = margin;
    const int xMatricule = xId + wId + 25;
    const int xMarque = xMatricule + wMatricule + 25;
    const int xKm = xMarque + wMarque + 25;
    const int xEtat = xKm + wKm + 25;

    // Helper function for text eliding
    auto elide = [&](const QString& s, int width, const QFont& f) {
        QFontMetrics fm(f);
        return fm.elidedText(s, Qt::ElideRight, width - 6);
    };

    // Title
    p.setFont(QFont("Arial", 14, QFont::Bold));
    p.drawText(margin, y, pageW, 40, Qt::AlignLeft | Qt::AlignVCenter,
               "Liste des Véhicules - NovaDrive");
    y += 45;
    ...
    auto drawHeader = [&](){
        ...
        p.setPen(Qt::white);
        p.setFont(QFont("Arial", 10, QFont::Bold));
        p.drawText(xId, y, wId, rowH, Qt::AlignVCenter, "ID");
        p.drawText(xMatricule, y, wMatricule, rowH, Qt::AlignVCenter, "Matricule");
        p.drawText(xMarque, y, wMarque, rowH, Qt::AlignVCenter, "Marque");
        p.drawText(xKm, y, wKm, rowH, Qt::AlignVCenter, "Kilométrage");
        p.drawText(xEtat, y, wEtat, rowH, Qt::AlignVCenter, "État");
        y += rowH;
        p.setPen(Qt::black);
        p.setFont(QFont("Arial", 10));
    };
    ...
    auto newPage = [&](){
        ...
        y = margin;
        p.setFont(QFont("Arial", 16, QFont::Bold));
        p.drawText(margin, y, pageW, 30, Qt::AlignLeft | Qt::AlignVCenter,
                   "Liste des Véhicules - NovaDrive");
        y += 38;
        drawHeader();
    };
    ...
    while (q.next()) {
        ...
        if (row % 2) {
            p.fillRect(QRect(margin, y, pageW, rowH), QColor(245, 245, 245));
        }

        QString id = q.value(0).toString();
        QString matricule = q.value(1).toString();
        QString marque = q.value(2).toString();
        QString km = q.value(3).toString() + " km";
        QString etat = q.value(4).toString();

        // Draw row data
        p.setFont(rowFont);
        p.drawText(xId, y, wId, rowH, Qt::AlignVCenter, elide(id, wId, rowFont));
        p.drawText(xMatricule, y, wMatricule, rowH, Qt::AlignVCenter,
                   elide(matricule, wMatricule, rowFont));
        p.drawText(xMarque, y, wMarque, rowH, Qt::AlignVCenter,
                   elide(marque, wMarque, rowFont));
        p.drawText(xKm, y, wKm, rowH, Qt::AlignVCenter, elide(km, wKm, rowFont));
        p.drawText(xEtat, y, wEtat, rowH, Qt::AlignVCenter, elide(etat, wEtat, rowFont));
...
void MainWindow::on_pushButton_email_clicked()
{
    ...
    QString km = ui->tableWidget->item(row, 3)->text();
    QString etat = ui->tableWidget->item(row, 4)->text();

    // Compose email
    QString subject = QString("Notification Véhicule - %1 (%2)").arg(marque, matricule);
    QString body = QString(
                       "Bonjour,\n\n"
                       "Notification concernant le véhicule suivant :\n\n"
                       "• Matricule : %1\n"
                       "• Marque : %2\n"
                       "• Kilométrage : %3\n"
                       "• État : %4\n\n"
                       ).arg(matricule, marque, km, etat);

    // Add maintenance alert if needed
    bool kmOk;
    int kmValue = km.split(" ").first().toInt(&kmOk);
    if (kmOk && kmValue > 100000) {
        body += "⚠️ ALERTE : Ce véhicule a dépassé 100 000 km.\n"
                "Une maintenance approfondie est recommandée.\n\n";
    }

    if (etat.contains("panne", Qt::CaseInsensitive)) {
        body += "⚠️ ATTENTION : Ce véhicule est actuellement en panne.\n"
                "Intervention technique requise.\n\n";
    }

    body += "Cordialement,\n"
            "Système de Gestion NovaDrive";

    // Open default email client (cross-platform)
    QString emailUrl = QString("mailto:?subject=%1&body=%2")
                           .arg(QString(QUrl::toPercentEncoding(subject)),
                                QString(QUrl::toPercentEncoding(body)));

    if (!QDesktopServices::openUrl(QUrl(emailUrl))) {
        // Fallback: show email content in dialog
        QMessageBox msgBox(this);
        msgBox.setWindowTitle("📧 Email Instructeur");
        msgBox.setIcon(QMessageBox::Information);
        msgBox.setText("Impossible d'ouvrir le client email.\n\nVoici le contenu à envoyer :");
        msgBox.setDetailedText(QString("Objet : %1\n\n%2").arg(subject, body));
